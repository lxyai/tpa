...
class TPASINGLELSTM:
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('LSTMNetWork'):
            ...
            self.lstm_inputs_embed = tf.unstack(self.lstm_inputs, axis=1)
            self.all_lstm_outputs, self.final_lstm_states = tf.nn.static_rnn(
                cell=self._build_single_cell(),
                inputs=self.lstm_inputs_embed,
                sequence_length=tf.constant([z_length for _ in range(self.para.batch_size)], dtype=tf.int32),
                dtype=self.dtype
            )
            self.future_lstm_outputs = tf.transpose(
                tf.stack(self.all_lstm_outputs[x_length:x_length+y_length], axis=0),
                [1, 0, 2]
            )
