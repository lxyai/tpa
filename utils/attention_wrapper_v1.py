...
class TemporalPatternAttentionMechanism():
    def __call__(self, query, attn_states, attn_size, attn_length,
        ...
        with tf.variable_scope("attention"):
            ...
            with tf.variable_scope('tpa_attention'):
                w = tf.reshape(
            ...
            with tf.variable_scope('temproal_attention'):
                m = tf.multiply(
                    tf.reshape(
                        dense(query, attn_size), [-1, 1, attn_size]
                    ),
                    attn_states
                )
                m_sum = tf.reduce_sum(m, [2])
                m_weight = tf.nn.softmax(m_sum)
                final_v = tf.reduce_sum(
                    tf.multiply(attn_states, tf.reshape(m_weight, [-1, attn_length, 1])),
                    [1]
                )
            final_new_attns = dense(tf.concat([new_attns, final_v], axis=1), attn_size)
            new_attn_states = tf.slice(attn_states, [0, 1, 0], [-1, -1, -1])
            return final_new_attns, new_attn_states
