...
class PolyRNN:
    ...
    def _build_graph(self):
        ...
        self.x = tf.placeholder(dtype=tf.float32, shape=[None, self.para.attention_len, self.para.output_size],
                                name='x')  # 此处手动将数据的宽度设置为321
        self.y = tf.placeholder(dtype=tf.float32, shape=[None, 1, self.para.output_size], name='y')  # 将y设为固定值

        self.rnn_inputs_embed = tf.nn.relu(tf.layers.dense(self.x, self.para.num_units))
        ...
        self.all_rnn_states, self.final_rnn_states = tf.nn.static_rnn(
            ...
            sequence_length=tf.constant([self.para.attention_len for i in range(self.para.batch_size)], dtype=tf.int32),
            dtype=self.dtype
        )
